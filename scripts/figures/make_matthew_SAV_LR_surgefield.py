...
Three panels: full domain, savannah region with satellite basemap, and tight
coupling-box zoom with basemap.
...
while i < len(lines):
    if 'grid_number' in lines[i]:
        ...
        for r in range(my):
            for c in range(mx):
                f = lines[j].split()
                h = float(f[0]); eta = float(f[3])
                vals[r, c] = eta if h > 0.01 else np.nan
                j += 1
            while j < len(lines) and lines[j].strip() == '':
                j += 1
...
if args.vmax is not None:
    vmax = args.vmax; vmin = -vmax
else:
    vmax = np.nanpercentile(allv, 99.5); vmin = -vmax

ZOOMS = [
    ('Full domain', False, None),
    ('Savannah region', True, 0.4),
    ('Coupling box (L6)', True, 0.02),
]

fig, axes = plt.subplots(1, 3, figsize=(22, 7),
    gridspec_kw={'width_ratios': [1, 1.2, 1.4]})

for ax, (title, do_zoom, pad) in zip(axes, ZOOMS):
    if do_zoom:
        ax.set_xlim(SAV_BOX[0]-pad, SAV_BOX[1]+pad)
        ax.set_ylim(SAV_BOX[2]-pad, SAV_BOX[3]+pad)
        if HAS_CTX:
            try:
    ...
    for lvl, x0, y0, dx, dy, mx, my, vals in sorted(patches, key=lambda d: d[0]):
        ...
        ax.pcolormesh(xe, ye, z, cmap='RdBu_r', vmin=vmin, vmax=vmax,
                      shading='flat', zorder=lvl, alpha=0.55)
